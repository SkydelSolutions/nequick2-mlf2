 ...
 * @file
 */
#include "NeQuick2_MLF2_Gauss_Kronrod_integration.h"

#include <stdio.h>

#include "NeQuickG_JRC_math_utils.h"
#include "NeQuick2_MLF2_ray_slant.h"
...
#define NEQUICK_G_JRC_KRONROD_MINIMUM_RECURSION 2
...
static bool is_error_within_tolerance(
  const NeQuick2_MLF2_gauss_kronrod_context_t* const pContext,
...
static int32_t get_total_electron_content(
  NeQuick2_MLF2_context_t* const pNequick_Context,
  double_t height_km,
  double_t* const pTotal_electron_content) {

  if (pNequick_Context->ray.is_vertical) {
    *pTotal_electron_content = NeQuick2_MLF2_ray_vertical_get_electron_density(
      pNequick_Context, height_km);
    return NEQUICK_OK;
  } else {
    return NeQuick2_MLF2_ray_slant_get_electron_density(
      pNequick_Context, height_km, pTotal_electron_content);
  }
}

int32_t NeQuick2_MLF2_Gauss_Kronrod_integrate(
  NeQuick2_MLF2_gauss_kronrod_context_t* const pContext,
  NeQuick2_MLF2_context_t* const pNequick_Context,
  ...
  *pResult = 0.0;
  ...
  if (is_error_within_tolerance(
    pContext,
    K15_integration,
    G7_integration) && pContext->recursion_level > NEQUICK_G_JRC_KRONROD_MINIMUM_RECURSION) {
    ...
    *pResult = K15_integration;
    ...
    ret = NeQuick2_MLF2_Gauss_Kronrod_integrate(
      pContext,
      pNequick_Context,
      point_1_height_km,
