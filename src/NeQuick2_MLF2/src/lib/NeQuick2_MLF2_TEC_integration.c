/** NeQuick2-MLF2 Total Electron Content (TEC) integration routine
 *
...
static double_t get_point_height(
  const NeQuick2_MLF2_context_t* const pContext,
  double_t height_km) {
...
static int32_t Gauss_Kronrod_integrate_impl(
  ...
  return NeQuick2_MLF2_Gauss_Kronrod_integrate(
    pContext,
    pNequick_Context,
    point_1_height_km,
    point_2_height_km,
...
static int32_t both_below_first_integration_point(
  ...
  NeQuick2_MLF2_gauss_kronrod_context_t gauss_kronrod_context;
...
int32_t NeQuick2_MLF2_integrate(
  NeQuick2_MLF2_context_t* const pContext,
  double_t* const pTEC) {
  ...
  *pTEC = 0.0;
  ...
  if (pContext->ray.is_vertical) {
    ret = NeQuick2_MLF2_ray_vertical_get_profile(pContext);
    if (ret != NEQUICK_OK) {
      return ret;
    }
  }
